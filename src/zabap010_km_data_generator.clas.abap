CLASS zabap010_km_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZABAP010_KM_DATA_GENERATOR IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA roomreservations TYPE STANDARD TABLE OF zraph_km_roomrsv WITH DEFAULT KEY."database Table

    " Clear personal travel table and fill it from /DMO/ reference table
    DELETE FROM zraph_km_travel.

    INSERT zraph_km_travel FROM ( SELECT * FROM /dmo/a_travel_d ).
    out->write( 'Travel data copied.' ).

    " Clear personal booking table and fill it from /DMO/ reference table
    DELETE FROM zraph_km_booking .

    INSERT zraph_km_booking FROM ( SELECT * FROM /dmo/a_booking_d ). out->write( 'Booking data copied.' ).

    " Clear personal booking supplement table and fill it from /DMO/ reference table
    DELETE FROM zraph_km_booksup.

    INSERT zraph_km_booksup FROM ( SELECT * FROM /dmo/a_bksuppl_d ).
    ...
    DELETE FROM zraph_km_roomrsv.

    SELECT travel_uuid, begin_date, end_date, total_price, currency_code
    FROM zraph_km_travel INTO TABLE @DATA(travels).
    ...
    LOOP AT travels ASSIGNING FIELD-SYMBOL(<travel>).
      ...
      ENDIF.
    ENDLOOP.
    INSERT zraph_km_roomrsv FROM TABLE @roomreservations.
